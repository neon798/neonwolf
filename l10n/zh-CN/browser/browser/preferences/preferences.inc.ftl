...
content-blocking-section-top-level-description = Neonwolf 默认支持并启用严格模式下的增强跟踪保护。这是浏览器中最重要的设置之一，因为它提供状态分区、严格的阻止列表和其他精妙的隐私功能。我们不建议更改为其他模式。
...
librewolf-rfp-warning =
    .message = 由于启用了防指纹跟踪（ResistFingerprinting），此功能已被禁用。这意味着 Neonwolf 会强制网页内容（默认）以浅色主题显示。
permissions-eme2 =
    .label = 受 DRM 控制的内容
