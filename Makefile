...
help :
	...
	@echo ""
	@echo "  all         - Make Neonwolf source archive ${version}-${release}."
	@echo ""
	...
	@echo "                ready to build neonwolf folder."
	@echo "  build       - Build Neonwolf (requires bootstrapped build environment)."
	@echo "  package     - Package Neonwolf (requires build)."
	@echo "  run         - Run Neonwolf (requires build)."
	@echo ""
	...
	@echo ""
	@echo "  patches   - Just make the Neonwolf source directory (download, extract, patch)"
...
check :
	...
	@echo "Neonwolf release : " $$(cat release)
	@echo ""
...
$(lw_source_dir) : $(ff_source_tarball) ./version ./release scripts/neonwolf-patches.py assets/mozconfig.new assets/patches.txt
...
$(lw_source_tarball) : $(lw_source_dir)
	...
	gzip --fast neonwolf-$(version)-$(release).source.tar
	touch $(lw_source_dir)
...
package :
	...
	cp -v $(lw_source_dir)/obj-*/dist/neonwolf-$(version)-$(release).en-US.*.tar.bz2 .
...
build_image=neonwolf-build-image
